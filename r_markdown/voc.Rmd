...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ...
  samples <- dplyr::select(samples, id, stove, fuel, fuelcat, stovecat, date)

  voc_merged <- dplyr::left_join(voc_merged, samples, by = "id") %>%
            dplyr::mutate(id= as.factor(id))
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=20, fig.height=250, fig.show='hide'}
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, results='markup', cache=FALSE, fig.width=20, fig.height=250, fig.show='hide'}
