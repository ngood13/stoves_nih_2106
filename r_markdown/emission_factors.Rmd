...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  emission_factors <- dplyr::left_join(emission_factors, 
               ...
               dplyr::mutate(mass_ef = ((value * 1e-6) / mass_fuel_kg),  # g / kg
                             mass_c_ef = ((value * 1e-6) / mass_fuel_c),  # g / kg
                             energy_ef = ((mass_ef * 1e-3) * lhv * 1e-3), # mg/mJ
                             energy_c_ef = ((mass_c_ef * 1e-3) * lhv * 1e-3))  # mg / MJ
...
ggplot(dplyr::filter(mass_emitted, fuel_type != "background"), aes(x = id, y = mass_ef, color = fuel_type)) +
    ...
    ggtitle("mass of pollutant emitted per mass of fuel burnt (g/kg)")
```
